...
inline fun <reified T : ViewModel> koinActivityViewModel(
    ...
    qualifier = qualifier,
    viewModelStoreOwner = LocalContext.current.findViewModelActivity() as ComponentActivity,
    key = key,
    scope = scope,
    parameters = parameters,
)

fun Context.findViewModelActivity(): Activity {
    var context = this
    while (context is ContextWrapper) {
        if (context is Activity) return context
        context = context.baseContext
    }
    error("Activity not found in current Context")
}